...
\[
    \p{\text{error}} = \int_{\fr_1}{\dba{x}{w_2}\p\wlii\id} + \int_{\fr_2}{\dba{x}{w_1}\p\wli\id}
\]
\begin{cimage}
...
\end{cimage}
\begin{example}{Classificare maschi/femmine in base a peso e altezza}{}
    \begin{center}
    ...
    \begin{itemize}
        ...
        \item \textbf{densità di probabilità condizionali} --- per un nuovo pattern $x$ da classificare: si conano le occorrenze dei pattern nel training set delle due classi in un intorno di $x$:
            \begin{itemize}
                \item $\dba{x}{\wli} = 1/8$;
...
%
\subsubsection{Distribuzione Normale $d=1$}\label{sec:distribuzione_normale_d_} % (fold)
...
\begin{example}{Stima di $\mu$ e $\si$ per $d = 1$}{}
    ...
    \end{warning}
\end{example}
% subsubsection Distribuzione Normale $d=1$ (end)
%
\subsubsection{Distribuzione Normale Multivariata}\label{sec:distribuzione_normale_multivariata} % (fold)
...
\indent
\begin{definition}{Distanza Mahalanobis}{}
...
% subsubsection Distribuzione Normale Multivariata (end)
%
\subsubsection{Classificatore di Bayes con distribuzioni Multinormali}\label{sec:classificatore_di_bayes_con_distribuzioni_multinormali} % (fold)
\begin{cimage}
...
\\
La classificazione è eseguita utilizzando la regola \textit{Bayesiana}: lo spazio è suddiviso in regioni non connesse, nel caso specifico $\fr_2$ è costituita da due componenti disgiunte.
\begin{definition}{Decision Boundary}{}
    ...
    \end{itemize}
\end{definition}
% subsubsection Classificatore di Bayes con distribuzioni Multinormali (end)
%
\subsubsection{Bayes e confidenza di classificazione}\label{sec:bayes_e_confidenza_di_classificazione} % (fold)
...
\]
% subsubsection Bayes e confidenza di classificazione (end)
\subsubsection{Bayes parametrico in pratica}\label{sec:bayes_parametrico_in_pratica} % (fold)
Molto spesso si fanno ipotesi azzardate sulla normalità delle densità di probabilità delle classi del problema senza aver sperimentalmente eseguito nessuna verifica; ciò porta ad ottenere \textbf{cattivi risultati} di classificazione.
\\
...
% subsubsection Bayes parametrico in pratica (end)
\subsubsection{Stima della Densità}\label{sec:stima_della_densita} % (fold)

% subsubsection Stima della Densita (end)
%
