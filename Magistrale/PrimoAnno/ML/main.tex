...
In un problema di Classificazione \cref{sec:classificazione}, l'accuratezza di classificazione $\ps{0\dots100\%}$ è la percentuale di pattern correttamente classificati.
...
\[
    RMSE = \sqrt{\frac{1}{N}\sum_{i = 1\dots N}{\pt{pred_i - true_i}^2}}
\]
%
\subsubsection{Matrice di Confusione}\label{sec:matrice_di_confusione} % (fold)
La matrice di confusione (confusion matrix) è molto utile nei problemi di classificazione \cref{sec:classificazione}, per capire come sono distribuiti gli errori.
