...
La probabilità che un pattern $x$ cada all'interno di $\fr$ è:
\[
    P_1 = \int_{\fr}{\dpb{x'}\,\id[x']}
\]
Dati $n$ pattern indipendenti, la probabilità che $k$ di questi cadano nella regione $\fr$ è calcolabile attraverso la \textbf{distribuzione binomiale}:
\[
    P_k = \binom{n}{k}P_1^k\pt{1 - P_1}^{n-k}
\]
il cui valore medio è $k = n \cdot P_1$, e quindi $P_1 = k / n$.
\emptyln{}
Assumendo che la regione $\fr$, di volume $V$, sia piccola e che quindi $\dpb{\cdot}$ non vari significativamente all'interno di essa:
\[
    P_k = \int_{\fr}\dpb{x'}\,\id[x'] \approx \dpb x \cdot V
\]
...
%
\subsubsection{Parzen Window}\label{sec:parzen_window} % (fold)
La regione $\fr$, denominata finestra (\textbf{Window}), è costituita da un ipercubo $d$-dimensionale, definito dalla funzione $\vphi$:
\[
    \fvphi{u} = \begin{cases}
...
% subsubsection Parzen Window (end)
% subsection Classificatore di Bayes (end)
%
\subsection{Nearest Neighbor}\label{sub:nn} % (fold)
...
%
\subsubsection{NN e Metriche}\label{sec:metriche} % (fold)
