...
\end{warning}
Per $TS$ infiniti la regola di classificazione $k$-NN si dimostra migliore di $1$-NN (\hyperref[sec:nn]{NN}), e all'aumentare di $k$, l'errore $P$ converge all'errore Bayesiano.
\begin{cimage}
...
\end{equation}
Nella pratica, prima di adottare semplicemente la distanza euclidea è bene valutare lo spazio di variazione delle componenti (o \red{feature}) e la presenza di eventuali forti correlazioni tra le stesse.
\\
...
\begin{quote}
    Il \textbf{margine} è la distanza minima di punti delle due classi nel training set dall'iperpiano individuato.
\end{quote}
\begin{cimage}
...
\begin{definition}{Iperpiano}{}
    ...
    \]
\end{definition}
\begin{definition}{SVM Margine}{svm-margine}
...
\begin{equation}\label{eq:vinc-svm-lin-sep}
    \begin{aligned}
        \text{Minimizza:}&\qquad \nm w ^2 / 2 \\
        \text{Vincoli:}&\qquad y_i\ps{w \cdot x_i + b} \ge 1 \qquad \text{per } i = 1,\,\dots,\,n
    \end{aligned}
\end{equation}
\begin{definition}{Support Vector}{sup-vec}
...
\end{definition}
\\
Il problema di ottimizzazione precedente, può essere risolto passando innanzitutto a una \textbf{formula Lagrangiana} e successivamente a una \textbf{formula duale}.
\\
La formulazione Lagrangiana prevede di introdurre un \red{moltiplicatore} $\al_i\ \pt{\al_i \ge 0}$ per ogni vincolo nella forma \textit{equazione} $ \ge 0$ e di sottrarre il vincolo moltiplicato per $\al_i$ della \hyperref[sec:parametri_e_funzione_obiettivo]{funzione obiettivo}:
\[
    Q\pt{w,\,b,\,\al} = \half w\cdot w - \sum_{i = 1}^n{\al_i\pt{y_i\ps{w \cdot x_i + b} - 1}}
\]
da \red{minimizzare} rispetto a $\mathbf{w}$ e $\mathbf{b}$ e \red{massimizzare} rispetto a $\boldsymbol{\al_i} \ge 0$.
\emptyln{}
Utilizzando le condizioni di \ac{kkt}, il problema può essere posto in \red{forma duale} esprimendo i parametri $w$ e $b$ in funzione dei moltiplicatori $\al_i$.
Si elimina così la dipendenza diretta da $w$ e $b$ e il problema può essere risolto massimizzando la nuova funzione obiettivo rispetto ai soli $\al_i$:
\begin{equation}\label{eq:kkt-cond-sep}
    \begin{aligned}
        Q\pt \al = &\sum_{i = 1,\,\dots,\,n}{\al_i}\, - \half \sum_{i,j = 1,\,\dots,\,n}{\al_i\al_j y_i y_j \pt{x_i\cdot x_j}} \\
        \text{con vincoli:}\quad &\sum_{i = 1,\,\dots,\,n}y_i\al_i = 0 \qquad \text{e} \qquad \al_i \ge 0 \quad \text{per } i = 1,\,\dots,\,n
    \end{aligned}
\end{equation}
\begin{note}
    I \textbf{pattern} ora compaiono solo come \red{prodotti scalari}
\end{note}
\emptyln{}
Ora è possibile utilizzare algoritmi di \textbf{programmazione quadratica} la cui soluzione consiste nel trovare i valori ottimi $\al[*]_1,\,\al[*]_2,\,\dots,\,\al[*]_n$.
Le condizioni \ac{kkt}, in \cref{eq:kkt-cond-sep}, assicurano che $\al[*]_i = 0$ per tutti i vettori che non sono support vector.
\\
L'iperpiano ottimo è dunque parametrizzato da:
\[
    \begin{aligned}
        w^* = &\sum_{i = 1,\,\dots,\,n}{\al[*]_iy_ix_i} \\
        b^* = y_s - &\sum_{i = 1,\,\dots,\,n}{\al[*]_iy_i\pt{x_i\cdot x_s}}
    \end{aligned}
\]
dove $\pt{x_s,\,y_s)}$ è uno dei \red{support vector}.
\\
La funzione distanza dall'iperpiano è:
\[
    D\pt x = w^* \cdot x + b^* = \sum_{i = 1,\,\dots,\,n}{\al[*]iy_i\pt{x\cdot x_i} + b^*}
\]
\begin{note}
    \begin{itemize}
        \item il \textbf{segno} della funzione $D\pt x$ consente di classificare un generico pattern $x$;
        \item le sommatorie sono riducibili ai soli support vector;
        \item nel caso lineare non è necessario, dopo aver calcolato $w^*$ e $b^*$, conservare o memorizzare i support vectors;
    \end{itemize}
\end{note}
\begin{quote}[Vantaggi]{}
    \begin{itemize}
        \item definizione della soluzione sulla base di un numero ridotto di \red{support vector};
        \item il numero di support vector $n_{sv}$ indica la complessità del problema e può essere dimostrato che l'errore medio, sui possibili $TS$, è limitato da $n_{sv}/n$;
        \item \ac{svm} <<scala>> molto bene rispetto alla dimensionalità $d$ dello spazio delle feature.
            La complessità computazionale del training è quadratica rispetto al numero $n$ di pattern in $TS$.
            In pratica il problema può essere risolto per $d = 10^7$ e per $n$ fino a $10^4$;
    \end{itemize}
\end{quote}
% subsubsection Pattern Separabili (end)
%
\subsubsection{Pattern non Separabili}\label{sec:pattern_non_separabili} % (fold)
In questo caso \red{non tutti} i pattern possono essere separati da un iperpiano, ed è necessario \textbf{rilassare i vincoli di separazione}, per far sì che alcuni pattern possano valicare il confine della classe.
A tal fine si introducono $n$ variabili di \textbf{slack} positive $\xi_i$, $i = 1,\,\dots,\,n$ e si modificano i vincoli di separazione:
\[
    y_i\ps{w\cdot x_i + b}\ge 1 - \xi_i \qquad \text{per } i = 1,\,\dots,\,n
\]
Per ogni pattern $x_i$ del $TS$, la variabile $\xi_i$ codifica la deviazione dal margine.
Per i pattern \red{separabili} del $TS$ le corrispondenti variabili di slack assumeranno valore $0$.
\begin{cimage}
    \includegraphics[width=.4\textwidth]{assets/svm-lin-non-sep-visual.png}
\end{cimage}
L'iperpiano ottimo deve in questo caso ancora \textbf{massimizzare il \hyperref[def:svm-margine]{margine}}, ma allo stesso tempo \textbf{minimizzare il numero} di elementi \textbf{non correttamente classificati}.
La \hyperref[eq:vinc-svm-lin-sep]{funzione obiettivo}, e di conseguenza il problema di ottimizzazione vengono così \red{modificati}:
\begin{equation}
    \begin{aligned}
        \text{Minimizza:}& \quad\dfrac{\nm w ^2}{2} + C\sum_{i = i,\,\dots,\,n}{\xi_i} \\
        \text{Vincoli:}& \quad y_i\ps{w\cdot x_i + b} \ge 1 - \xi_i \quad \text{per }i = 1,\,\dots,\,n
    \end{aligned}
\end{equation}
\\
Il coefficiente $C$ nel problema di ottimizzazione precedente, indica l'\red{importanza relativa} degli errori di classificazione rispetto all'ampiezza del margine.
Si tratta di \textbf{uno dei pochi iperparametri} che l'utente deve scegliere per il tuning di \textbf{\ac{svm}}.
\emptyln{}
Passando attraverso forma lagrangiana/duale otteniamo un risultato uguale al caso linearmente separabile, \cref{eq:kkt-cond-sep}, \red{tranne} che per l'introduzione del \textbf{limite superiore} $C$ per io valori dei moltiplicatori $\al_i$:
\begin{equation}\label{eq:kkt-cond-non-sep}
    \begin{aligned}
        Q\pt \al = &\sum_{i = 1,\,\dots,\,n}{\al_i}\, - \half \sum_{i,j = 1,\,\dots,\,n}{\al_i\al_j y_i y_j \pt{x_i\cdot x_j}} \\
        \text{con vincoli:}\quad &\sum_{i = 1,\,\dots,\,n}y_i\al_i = 0 \qquad \text{e} \qquad 0 \le \al_i \le C \quad \text{per } i = 1,\,\dots,\,n
    \end{aligned}
\end{equation}
Il metodo di soluzione e il modo di derivare l'iperpiano dagli $\al_i$ sono gli stessi del caso linearmente separabile.
\begin{cimage}
    \includegraphics[width=.8\textwidth]{assets/lin-svm-non-sep-es.png}
    \caption{Esempio al variare di $C$}\label{fig:lin-svm-non-sep-es}
\end{cimage}
% subsubsection Pattern non Separabili (end)
% subsection SVM Lineari (end)
%
\subsection{SVM Non Lineari}\label{sec:svm_non_lineari} % (fold)
\ac{svm} prevede un'importante estensione della teoria inizialmente sviluppata per iperpiani, al caso di \textbf{separazione dei pattern} con superfici anche molto complesse (\red{non lineari}).
Tutto ciò avviene in questo modo:
\begin{itemize}
    \item viene definito un \red{mapping} $\Phi$ non lineare dei pattern dallo spazio di partenza $\Re^d$ verso uno spazio $\Re^m$ a più alta dimensionalità ($m > d$):
        \[
            \Phi:\Re^d \to\Re^m,\, \fPhi x = \ps{\gix,\,\gix[2],\,\dots,\,\gix[m]}
        \]
    \item nello spazio $\Re^m$, dove \textbf{maggiori sono i gradi di libertà}, i pattern $\fPhi{x_1},\,\fPhi{x_2},\,\dots,\,\fPhi{x_n}$ possono essere più facilmente \textbf{separati} da un \textbf{iperpiano} utilizzando la teoria nota.
        Ciò equivale a separare i pattern $x_1,\,x_2,\,\dots,\,x_n$ in $\Re^d$ con \red{superfici} arbitrariamente \red{complesse}.
\end{itemize}
Analizzando la formulazione del problema lagrangiano-duale, si nota che i vettori del $TS$ appaiono solo in forma di \textbf{prodotti scalari} tra coppie di vettori.
Questa proprietà permette di \textbf{evitare la manipolazione} di vettori nello spazio $\Re^m$.
\emptyln{}
Infatti, per opportuni mapping $\Phi$ è possibile ricondurre il prodotto scalare di due pattern mappati nello spazio $\Re^m$ a una funzione $K$, detta \red{Kernel}, dei due pattern originali nello spazio $\Re^d$:
\[
    \fPhi{x} \cdot \fPhi{x'} = K\pt{x,\,x'}
\]
\begin{note}
    Per il \red{teorema di Mercer} se la funzione Kernel $K$ soddisfa alcuni requisiti (es. è continua e simmetrica rispetto agli argomenti), allora esiste un mapping $\Phi$ verso uno spazio di dimensionalità maggiore che gode della suddetta proprietà.
    È dunque possibile utilizzare $K$ senza nemmeno conoscere il corrispondente $\Phi$.
\end{note}
%
\subsubsection{Kernel Functions}\label{sec:kernel_functions} % (fold)
\paragraph{Polinomio di grado $q$ (iperparametro)}\label{par:polinomio_di_grado_q_iperparametro_} % (fold)
Le componenti $\gix[i]$, $i = 1,\,\dots,\,m$ sono ottenute come tutte le possibili combinazioni di elevamento a potenze $\le q$ delle componenti di $x$.
\\
Ad esempio per $d = 2$, $q = 2$:
\[
    \fPhi{x} = \fPhi{\ps{x_1, x_2}} \break = \ps{1,\, x_1,\, x_2,\, x_1x_2,\, x_1^2,\, x_2^2,\, x_1^2x_2,\, x_1x_2^2,\, x_1^2x_2^2}
\]
e quindi $m = 9$.
Si dimostra che:
\[
    K\pt{x,\,x'} = \ps{\pt{x\cdot x'} + 1}^q
\]
% paragraph Polinomio di grado q (iperparametro) (end)
\paragraph{Radial Basis Function (RBF)}\label{par:radial_basis_function} % (fold)
di ampiezza $\si$ (iperparametro)
\[
     K\pt{x,\,x'} = e^{\pt{-\dfrac{\nm{x-x'}^2}{2\si[2]}}}
\]
% paragraph Radial Basis Function (RBF) (end)
\paragraph{Sigmoid}\label{par:sigmoid} % (fold)
Meno utilizzato:
\[
    K\pt{x,\,x'} = \tanh{\pt{v\pt{x\cdot x'}+a}}
\]
$v$ e $a$ (iperparametri) devono essere scelti opportunamente: una possibile scelta è $v = 1$, $a = 1$.
% paragraph Sigmoid (end)
\emptyln{}
Utilizzando il \red{kernel trick} si può risolvere il problema di ottimizzazione senza particolari complicazioni rispetto al caso lineare.
Una volta determinati gli $\al[*]_i$, la \red{superficie di separazione}, regola di classificazione, è esprimibile come:
\[
    D\pt x = \sum_{i = 1,\,\dots,\,n}{\al[*]_i y_i\red{K\pt{x,\,x_i}} + b^*}
\]
\begin{cimage}
    \includegraphics[width=.3\textwidth]{assets/svm-no-lin-es.png}
    \caption{}\label{fig:svm-no-lin-es}
\end{cimage}
% subsubsection Kernel Functions (end)
% subsection SVM Non lineari (end)
%
\subsection{SVM Multiclasse}\label{sec:svm_multiclasse} % (fold)
\ac{svm} è in grado di determinare la \red{superficie di separazione} tra $2$ classi di pattern, nel caso in cui si voglia gestire problemi con più di due classi esistono diverse soluzioni.
Le più comuni sono:
\begin{itemize}
    \item \textbf{One-Against-One};
    \item \textbf{One-Against-All};
\end{itemize}
\begin{definition}{SVM One-Against-All}{svm-one-ag-all}
    Date $s$ classi $w_1,\,w_2,\,\dots,\,w_s$.
    \\
    Per ogni classe $w_k$, si determina con \ac{svm} la superficie di separazione tra i pattern di $w_k$, etichettati $+1$, da una parte e i pattern di tutte le rimanenti classi $w_h$, $h \ne k$ etichettati $-1$, dall'altra.
    \\
    Si ottiene la funzione $D_x\pt x$ che indica quanto $x$ è distante dalla superficie decisionale in direzione di $w_k$; \textbf{maggiore} è $D_k\pt x$ più confidenti siamo dell'appartenenza di $x$ a $w_k$.
    \emptyln{}
    Al termine del training, si assegna il pattern $x$ alla classe $k^*$ per cui è massima la distanza dalla superficie decisionale:
    \[
        k^* = \argmax_k{\pg{D_k\pt x}}
    \]
    \begin{note}
        È necessario eseguire $s$ training \ac{svm}.
    \end{note}
\end{definition}
% subsection SVM multiclasse (end)
\subsection{SVM Implementazione}\label{sec:svm_implementazione} % (fold)
Il training di \ac{svm}, richiede \red{algoritmi numerici non banali} in grado di risolvere un problema di programmazione quadratica.
Link ad alcune implementazioni online:
\begin{itemize}
    \item \href{https://www.csie.ntu.edu.tw/~cjlin/libsvm/}{LIBSVM} --- viene utilizzato il parametro $\gamma$ per regolare la complessità della superficie decisionale: aumentando $\gamma$ la superficie può assumere forme più complesse;
    \item \href{https://www.csie.ntu.edu.tw/~cjlin/liblinear/}{LIBLINEAR} --- consigliata nel caso lineare per elevata dimensionalità ed elevato numero di pattern;
    \item \href{http://svmlight.joachims.org}{SVM-light}
\end{itemize}
% subsection SVM Implementazione (end)
\subsubsection{SVM In Pratica}\label{sec:svm_in_pratica} % (fold)
\begin{note}[Quando usare \ac{svm}?]
    \ac{svm} permette di ottenere buoni risultati su dati numerici \red{omogenei}, ma è meno performante di multi-classificatori basati su alberi decisionali su dati \red{tabulari eterogenei}.
\end{note}
\begin{note}[Lineare o Non Lineare?]
    \begin{itemize}
        \item Se la dimensionalità $d$ dello spazio è molto elevata (es. 5000 feature) si utilizza generalmente \red{\ac{svm} lineare}.
            Infatti in uno spazio così grande i pattern sono tipicamente molto sparsi e anche <<semplici>> iperpiani sono in grado di separare le classi efficacemente.
            \\
            Il solo parametro da tarare è $C$.
        \item Per \red{bassa dimensionalità} (es. 20 feature) la scelta primaria è \red{\ac{svm} non lineare} con \textbf{kernel RBF}.
            \\
            Gli iperparametri da tarare sono $C$ e $\si$ (o $\gm$ se si utilizza LIBSVM).
        \item Per media dimensionalità (es. 200 feature) in genere si provano entrambe le tipologie.
            \\
            Come sempre gli iperparametri si tarano su un \textbf{validation set separato}, oppure attraverso \hyperref[par:k-fold-cross-val]{cross-validation} sul training set.
    \end{itemize}
\end{note}
\begin{note}[Come gestire il caso multi-classe?]
    Tipicamente ci si affida alla soluzione disponibile nella libreria utilizzata (\textbf{One-Against-One} per LIBSVM).
    Se però il numero di classi è molto elevato, il costo può diventare inaccettabile per certe applicazioni.
    \\
    In questo caso \textbf{One-Against-All} diventa la scelta obbligatoria.
\end{note}
% subsubsection SVM In Pratica (end)
% section SVM (end)
%
\section{Multi classificatori}\label{sub:multi_classificatori} % (fold)
Un \textbf{multi-classificatore} è un approccio dove diversi classificatori sono utilizzati, normalmente in parallelo ma talvolta anche in cascata o in modo gerarchico, per eseguire la classificazione dei pattern.
Le \red{decisioni} sei singoli classificatori sono \textbf{fuse} a un certo livello del processo di classificazione.
\emptyln{}
È stato dimostrato che l'utilizzo di combinazioni di classificatori può migliorare, anche molto, le prestazioni.
\begin{warning}
    La combinazione è efficace solo quando i singoli classificatori sono \textit{indipendenti tra loro}, ovvero non commettono gli stessi errori.
\end{warning}
L'\textbf{indipendenza} è normalmente ottenuta:
\begin{itemize}
    \item utilizzando feature diverse (non o poco correlate);
    \item utilizzando algoritmi diversi per l'estrazione delle feature;
    \item utilizzando diversi algoritmi di classificazione;
    \item addestrando lo stesso algoritmo di classificazione su porzione diverse del training set (\textbf{bagging});
    \item insistendo con l'addestramento sugli errori commessi dai predecessori (\textbf{boosting});
\end{itemize}
La combinazione può essere eseguita a \red{livello di decisione} o a \red{livello di confidenza}.
%
\subsection{Fusione a livello di decisione}\label{sec:fusione_a_livello_di_decisione} % (fold)
Un singolo classificatore fornisce in output la propria \red{decisione} che consiste nella classe cui ha assegnato il pattern.
Le decisioni possono essere tra loro \textbf{combinate} in diversi modi, tra cui:
\begin{itemize}
    \item \textbf{Majority vote rule}: è uno dei più noti e semplici metodi di fusione.
        Ogni classificatore vota per una classe, il pattern viene assegnato alla classe maggiormente votata.
        Più formalmente:
        \begin{theorem}{Majority Vote Rule}{maj-vote-rule}
            Se $\pg{C_1,\,C_2,\,\dots,\,C_{nc}}$ è un insieme di $nc$ classificatori, e
            \[
                \te_{ij} = \begin{cases}
                    1 & \text{se $i$ è la classe votata da $C_j$} \\
                    0 & \text{altrimenti}
                \end{cases}
                \qquad 1 \le i \le s,\, 1 \le j \le nc
            \]
            Allora il pattern è assegnato alla classe $t$ tale che:
            \[
                t = \argmax_{i=1,\,\dots,\,s}{\pg{\sum\nolimits_{j=1,\,\dots,\,nc}{\te_{ij}}}}
            \]
        \end{theorem}
    \item \textbf{Borda count}: ogni classificatore invece di una \red{singola classe}, produce una \textbf{classifica} o \textbf{ranking} delle classi a seconda della probabilità che a ciascuna di esse appartenga il pattern da classificare.
        \emptyln{}
        I \red{ranking} sono convertiti in punteggi e poi \red{sommati}; la classe con il più elevato punteggio finale è quella scelta dal multi-classificatore.
        \\
        Rispetto a majority vote rule, considera anche i <<non primi>> di ogni classificatore.
\end{itemize}
%
\subsubsection{Prestazioni Teoriche}\label{sec:prestazioni_teoriche} % (fold)
Dato un problema di classificazione binario e un \red{multi-classificatore} costituito da $nc$ classificatori ($nc$ dispari), la \hyperref[th:maj-vote-rule]{majority vote rule} classifica il pattern come appartenente alla classe che ha ottenuto almeno $k = \pt{nc + 1} / 2$ voti.
\\
Sia $P$ la probabilità che ogni singolo classificatore esegua la classificazione correttamente, la probabilità $P_{multi}\pt{nc}$ che la \red{decisione del multi-classificatore} sia corretta è esprimibile attraverso la \textbf{distribuzione binomiale}:
\[
    P_{multi}\pt{nc} = \sum_{m=k}^{nc}{\binom{nc}{m}}P^m\pt{1-P}^{nc-m}
\]
\begin{note}
    La distribuzione binomiale permette di calcolare con quale probabilità estraiamo $m$ palline rosse da un'urna contenente $nc$ palline, se $P$ è la probabilità di estrarne una rossa in un singolo tentativo.
\end{note}
Nella formula di $P_{multi}\pt{nc}$ i casi favorevoli sono quelli in cui un numero di classificatori compreso tra $k$ e $nc$ hanno effettuato classificazione corretta.
\begin{example}{}{}
    Se $P = 0.8$ (ogni classificatore commette il $20\%$ di errore di classificazione) stando alla formula precedente per un multi-classificatore con $nc$ componenti:
    \[
        \begin{aligned}
            &P_{multi}(3) = 0.896 \qquad &\pt{k = 2} \\
            &P_{multi}(5) = 0.942 \qquad &\pt{k = 3} \\
            &P_{multi}(7) = 0.966 \qquad &\pt{k = 4} \\
            &P_{multi}(9) = 0.980 \qquad &\pt{k = 5} \\
            &P_{multi}(15) = 0.995 \qquad &\pt{k = 8} \\
            &P_{multi}(21) = 0.999 \qquad &\pt{k = 11} \\
        \end{aligned}
    \]
    \begin{center}
        \includegraphics[width=.8\textwidth]{assets/maj-vote-es-1.png}
    \end{center}
    \begin{center}
        \includegraphics[width=.8\textwidth]{assets/maj-vote-es-1-visual.png}
        \captionof{figure}{Il training set è diviso in $3$ parti e addestrato da $3$ classificatori}\label{fig:maj-vote-es-1-visual}
    \end{center}
\end{example}
%
% subsubsection Prestazioni Teoriche (end)
\subsubsection{One-Against-One}\label{sec:one_against_one} % (fold)
L'approccio \red{One-Against-One}, consente di risolvere un problema di classificazione multi-classe, attraverso classificatori binari.
È l'approccio adottato dalla libreria LIBSVM.
\\
Se $s$ sono le classi del problema, si addestrano $s \times\pt{s-1}/2$ classificatori binari: \textbf{tutte le possibili coppie indipendentemente dall'ordine}.
\emptyln{}
...
Al termine il pattern $x$ è assegnato alla classe che ha ricevuto più voti (\hyperref[th:maj-vote-rule]{majority vote rule}).
\emptyln{}
È in genere più accurato di \hyperref[def:svm-one-ag-all]{One-Against-All} anche se meno efficiente in quanto richiede l'addestramento di un numero maggiore di classificatori.
% subsubsection One-Against-One (end)
...
Ogni singolo classificatore $C_j,\,j= 1,\,\dots,\,nc$ fornisce in output la confidenza di classificazione del pattern rispetto a ciascuna delle classi, ovvero un vettore $conf_i = \ps{conf_{j1},\,conf_{j2},\,\dots,\,conf_{js}}$ in cui l'$i$-esimo elemento indica il grado di appartenenza del pattern alla classe $i$-esima.
\\
Diversi metodi di fusione sono possibili tra cui:
\begin{itemize}
    \item \textbf{somma}: $\sum_{j=1,\,\dots,\,nc}{conf_j}\qquad t = \argmax_{i=1,\,\dots,\,s}{\pg{sum_i}}$
    \item \textbf{prodotto}: $\prod_{j=1,\,\dots,\,nc}{conf_j}\qquad t = \argmax_{i=1,\,\dots,\,s}{\pg{prod_i}}$
    \item \textbf{massimo}: $\max_{j=1,\,\dots,\,nc}{\pg{conf_{ji}}}\qquad t = \argmax_{i=1,\,\dots,\,s}{\pg{max_i}}$
    \item \textbf{minimo}: $\min_{j=1,\,\dots,\,nc}{\pg{conf_{ji}}}\qquad t = \argmax_{i=1,\,\dots,\,s}{\pg{min_i}}$
\end{itemize}
\begin{note}
    Il criterio del minimo può sembrare illogico: \textbf{viene scelto il massimo dei minimi}; quindi una classe che non ha ricevuto confidenza troppo bassa da nessun classificatore.
\end{note}
Il prodotto è il metodo <<probabilisticamente>> più corretto ma solo nel caso di indipendenza statistica.
\\
Nella pratica la \textbf{somma è spesso preferibile al prodotto} in quanto più robusta.
Infatti nel prodotto è sufficiente che un solo classificatore indichi confidenza zero per una classe per portare a zero la confidenza del multi-classificatore per quella classe.
\emptyln{}
Una \red{variante efficace}, è quella della \textbf{somma pesata}, dove la somma dei vettori di confidenza è eseguita pesando i diversi classificatori in base al loro grado di agilità $g_j$.
\[
    avgsum = \sum_{\for[j][][nc]}{g_j \cdot conf_j} \qquad t = \argmax_{\for[][][s]}{\pg{avgsum_i}}
\]
I gradi di libertà possono essere definiti in base alle singole prestazioni dei classificatori, ad esempio in maniera \textbf{inversamente proporzionale all'errore di classificazione}.
\begin{warning}
    Attenzione alla \textbf{normalizzazione} dei vettori di confidenza nel caso in cui essi non siano probabilità
\end{warning}
% subsection Fusione a livello di confidenza (end)
%
\subsection{Random Forest (Bagging)}\label{sec:random_forest_bagging_} % (fold)
Ideato da Leo Breiman nel 2001, appartiene alla famiglia di metodi di \ac{bgg}:
\begin{itemize}
    \item estrazione con re-imbussolamento di un sottoinsieme $S_j$ di pattern dal training set (tipicamente i $2/3$ del totale);
    \item addestramento del classificatore $C_j$ sul sottoinsieme $S_j$;
    \item fusione dei classificatori;
\end{itemize}
In \red{Random Forest} i singoli classificatori sono \textbf{classification tree} (alberi di classificazione).
\begin{definition}{Classification Tree}{class-tree}
    I \red{classification tree} sono uno strumento molto utilizzato in applicazioni con pattern categorici o mixed (come Data Mining).
    Per pattern numerici <<puri>> in genere non sono competitivi con \ac{svm} se considerati singolarmente.
    \\
    Esistono diversi tipi di alberi, tra cui: ID3, C4.5, CART (default in Random Forest).
\end{definition}
\red{Cart} è un albero binario in cui ogni nodo divide i pattern del training set, assegnandoli ai due nodi figli.
La suddivisione si bassa sul confronto di una singola feature con una soglia.
\emptyln{}
Per la <<crescita>> dell'albero a partire da un training set, si sceglie, in modo greedy, a ogni livello la coppia che meglio separa le classi.
Per la \textbf{classificazione} di un nuovo pattern si visita l'albero e una volta giunti a una foglia, si classifica il pattern sulla base della classe più comune nel nodo: \hyperref[th:maj-vote-rule]{majority vote rule}.
\begin{cimage}
    \includegraphics[width=.6\textwidth]{assets/class-tree-es.png}
    \caption{In numero le probabilità di sopravvivenza (sinistra) e la percentuale di campioni nella foglia (destra)}\label{fig:class-tree-es}
\end{cimage}
\begin{example}{Apprendimento di un albero CART}{}
    Per ogni nodo, a partire dalla radice, l'algoritmo suddivide gli $m$ pattern entranti del training set in due sottoinsiemi disgiunti di dimensioni $m_{left}$ e $m_{right}$.
    \\
    La suddivisione avviene in modo \red{greedy} considerando una \textbf{singola feature $k$} e una \textbf{soglia $t_k$}.
    La coppia $\pt{k,t_k}$ è scelta come quella che produce sottoinsiemi più \red{puri}, minimizzando:
    \[
        J\pt{k,t_k} = \frac{m_{left}}{m}G_{left} + \frac{m_{right}}{m}G_{right}
    \]
    $G_{left}$ e $G_{right}$ sono le \textbf{impurità} dei due sottoinsiemi.
    Per calcolare le impurità si può usare \textbf{Gini} o \textbf{Entropia}:
    \[
        \textit{Gini} = 1 - \sum_{i=1}^s{p_i^2}
    \]
    \[
        \textit{Entropia} = - \sum_{i=1}^s{p_i\log_2\pt{p_i}}
    \]
    dove $p_i$ sono le probabilità delle classi.
    \begin{note}
        Le due metriche portano a risultati spesso molto simili: la prima è usata come default perché più efficiente da calcolare
    \end{note}
    Il processo \textbf{continua ricorsivamente}, splittando i sottoinsiemi creati fino a quando:
    \begin{itemize}
        \item si raggiunge in ogni ramo la profondità massima specificata da iperparametro $max_depth$;
        \item non è possibile \textbf{ridurre ulteriormente} le impurità;
    \end{itemize}
\end{example}
In Random Forest, per rendere maggiormente indipendenti i classificatori: per ogni nodo la scelta della feature migliore su cui partizionare non è fatta sull'intero insieme delle $d$ feature (dimensionalità dei pattern), ma su un sottoinsieme random di $d'$ (si solito $d' = \sqrt d$).
\\
In assenza di questo accorgimento, noto anche come \red{feature bagging}, molti tree sceglierebbero con elevata probabilità le stesse variabili.
Pertanto Random Forest opera simultaneamente due tipi di bagging: uno sui pattern del training set e uno sulle features.
\emptyln{}
Il numero di tree (iperparametro \texttt{n\_estimators}) in una forest varia generalmente da alcune centinaia ad alcune migliaia.
Aumentare \texttt{n\_estimators} oltre al valore ottimale in genere non produce overfitting, ma rende il sistema meno efficace.
\emptyln{}
Gli alberi possono essere fatti crescere fino a quando lo split dei nodi termina naturalmente oppure imponendo una massima profondità (iperparametro \texttt{max\_depth}).
Ridurre \texttt{max\_depth} ha effetto \red{regolarizzante} e riduce l'overfitting.
\emptyln{}
Grazie al bagging, le prestazioni possono essere stimate con tecnica \ac{oob} che non richiede validation set separato: per ogni pattern calcola la prediction media di tutti gli alberi che non includono tale pattern nel loro bagging set.
% subsection Random Forest (Bagging) (end)
%
\subsection{AdaBoost (Boosting)}\label{sec:adaboost_boosting_} % (fold)
Ideato da \textbf{Freund} e \textbf{Schapire}, appartiene alla famiglia dei metodi di \textbf{boosting}, dove: più classificatori sono, tipicamente \red{deboli}, sono combinati per realizzare un classificatore \red{forte}.
\\
A differenza del bagging, l'apprendimento è \textbf{incrementale} e prevede di aggiungere a ogni iterazione un classificatore efficace sui pattern specifici.
A ogni iterazione si assegna un peso a ciascun pattern del training set.
Pattern classificati erroneamente hanno peso maggiore e contribuiscono maggiormente nella selezione del prossimo classificatore.
\emptyln{}
Siano:
\begin{itemize}
    \item $\pt{x_1,\,y_1},\,\dots,\,\pt{x_n,\,y_n}$ i pattern del $TS$ e le rispettive etichette, con $y_i \in \pg{+1,\,-1}$;
    \item $w_i^t$ il peso del pattern $x_i$ all'iterazione $t$;
    \item $h^t\pt x \to \pg{+1,\,-1}$ il classificatore introdotto all'iterazione $t$ e scelto tra i classificatori $h_j^t\pt x$ disponibili all'iterazione $t$;
    \item $\al[t]$ il peso del classificatore $h^t\pt x$ nell'ambito del multi-classificatore;
\end{itemize}
\subsubsection{Pseudocodice}\label{sec:pseudocodice} % (fold)

\begin{cimage}
    \includegraphics[width=.8\textwidth]{assets/adaboost-pseudo.png}
    \caption{Pseudocodice dell'algoritmo AdaBoost}\label{fig:adaboost-pseudo}
\end{cimage}
Al termine dell'addestramento, il \red{classificatore finale} (strong) è la media pesata dei weak classifiers:
\[
    H\pt x = \sign{\pt{\sum_{\for[t][][T]}{\al[t]h^t\pt x}}}
\]
%
% subsubsection Pseudocodice (end)
\subsubsection{Scelta dei pesi}\label{sec:scelta_dei_pesi} % (fold)
\[
    \al[t] = \half ln\pt{\frac{1 - \eps[t]}{\eps[t]}}
\]
\begin{cimage}
    \includegraphics[width=.4\textwidth]{assets/adaboos-weights-visual.png}
\end{cimage}
Un classificatore binario con errore $0.5$ ha peso $0$ (neutro), il peso aumenta esponenzialmente quando l'errore si avvicina a $0$ e diventa negativo se l'errore è superiore a $0.5$.
\\
\[
    w_i^{\inc t} = w_i^t \cdot e^z \qquad \text{dove } z = -\al[t]y_ih^t\pt{x_i}
\]
osservando che:
\[
y_ih^t\pt{x_i} = \begin{cases}
    1 & \text{se il pattern è correttamente classificato} \\
    -1 & \text{altrimenti}
\end{cases}
\]
\begin{itemize}
    \item $w_i^{\inc t}$ aumenta se $z < 0$: $\al[t]$ positivo e classificazione errata, oppure $\al[t]$ negativo e classificazione corretta;
    \item $x_i^{\inc t}$ diminuisce se $z < 0$: nei 2 casi complementari;
\end{itemize}
%
% subsubsection Scelta dei pesi (end)
% subsection AdaBoost (Boosting) (end)
%
\subsection{Gradient Boosting}\label{sec:gradient_boosting} % (fold)
È una tecnica di boosting che invece di assegnare peso maggiore agli hard examples, cerca di addestrare i nuovi weak models sui residuali di errore.
\\
L'implementazione più semplice si applica al caso di \red{regressione con loss MSE}, maggiori dettagli sono forniti nelle slide sulla di regressione.
