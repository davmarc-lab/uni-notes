...
Dato un problema di classificazione binario e un \red{multi-classificatore} costituito da $nc$ classificatori ($nc$ dispari), la \link[th:maj-vote-rule]{majority vote rule} classifica il pattern come appartenente alla classe cha ha ottenuto almeno $k = \pt{nc + 1} / 2$ voti.
\\
...
Al termine il pattern $x$ è assegnato alla classe che ha ricevuto più voti (\link[th:maj-vote-rule]{majority vote rule}).
\emptyln{}
È in genere più accurato di \link[def:svm-one-ag-all]{One-Against-All} anche se meno efficiente in quanto richiede l'addestramento di un numero maggiore di classificatori.
...
Per la \textbf{classificazione} di un nuovo pattern si visita l'albero e una volta giunti a una foglia, si classifica il pattern sulla base della classe più comune nel nodo: \link[th:maj-vote-rule]{majority vote rule}.
\begin{cimage}
