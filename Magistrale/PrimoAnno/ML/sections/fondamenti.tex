...
%
\paragraph{K-fold Cross-Validation}\label{par:k-fold-cross-val} % (fold)
...
\begin{example}{Esempio con funzione \texttt{GridSearchCV}}{}
    ...
    \begin{itemize}
        ...
        \item in totale $12$ valutazioni, ciascuna delle quali fatta con Cross Validation;
    \end{itemize}
\end{example}
\begin{cimage}
...
\begin{example}{}{}
    ...
    Sia $t$ il valore di soglia, allora il pattern viene classificato come positivo se $p > t$, come negativo in caso contrario.
\end{example}
% subsection Problemi Closed e Open set (end)
%
\subsection{Anomaly Detection}\label{sub:anomaly_detection} % (fold)
L'obiettivo è quello di capire se si sta verificando una \textbf{situazione anomala prima che sia troppo tardi}.
Raramente si hanno a disposizione sufficienti dati per descrivere i casi di anomalia, mentre sono disponibili molti dati del funzionamento normale.
\emptyln{}
Allora si può approcciare il problema \textbf{apprendendo} in modo non supervisionato la singola classe del funzionamento normale.
Dopo l'addestramento il modello analizza lo stato del sistema e produce un allarme, chiamata \textbf{anomalia}, quando questo si \textbf{discosta} dal funzionamento normale.
\begin{example}{}{}
    Supponiamo che la seguente nuvola di punti neri (\textbf{vibrazioni}, \textbf{assorbimento}) caratterizzi il funzionamento normale di una macchina utensile.
    \begin{center}
        \includegraphics[width=.8\textwidth]{assets/anom-det-ex.png}
    \end{center}
\end{example}
% subsection Anomaly Detection (end)
%
...
\begin{critical}
    \begin{itemize}
        \item se la \textbf{loss} non decresce (od oscilla significativamente) il sistema non converge: 
            \begin{itemize}
