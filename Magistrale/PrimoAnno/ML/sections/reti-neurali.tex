...
%
\section{Multilayer Perceptron}\label{sec:mlp} % (fold)
...
\begin{quote}[Cosa si intende per mapping desiderato?]
    Se la rete neurale deve operare come un \hyperref[chap:classificazione]{classificatore}, l'output desiderato è l'etichetta corretta della classe del pattern in input.
    Se la rete deve risolvere un problema di \hyperref[chap:regressione]{regressione}, l'output desiderato è il valore corretto della variabile dipendente, in corrispondenza del valore della variabile indipendente fornita in input.
\end{quote}
...
L'addestramento supervisionato avviene presentando alla rete pattern di cui è nota la classe e propagando (\hyperref[sub:mlp_forward_propagation]{forward propagation}) gli input verso gli output attraverso l'\cref{eq:forw-prop}.
\\
...
\end{cimage}
\begin{note}
    La \hyperref[sub:convergenza]{convergenza} si valuta monitorando l'andamento del loss e l'accuratezza sul validation set
\end{note}
...
\begin{itemize}
    ...
    \item \hyperref[par:sigmoid]{sigmoid}: i valori di uscita sono compresi nell'intervallo $\itc{0,\,1}$, ma non abbiamo nessuna garanzia che la somma sui neuroni di uscita sia $1$ (requisito fondamentale affinché si possano interpretare come distribuzione probabilistica):
        \[
            \sum_{\for[c][][s]}{z_c \ne 1}
        \]
\end{itemize}
...
\begin{quote}
    \begin{itemize}
        ...
        \item La rete neurale è in grado di trovare un mapping non lineare tra input e output, pertanto il termine \hyperref[sec:lineare]{linear} non si applica;
    \end{itemize}
\end{quote}
...
\begin{quote}
    Come \red{loss function} si può usare una qualsiasi fra: \hyperref[def:rmse]{\ac{mse}}, \hyperref[def:mae]{\ac{mae}}, \hyperref[def:mape]{\ac{mape}}
\end{quote}
...
Ciò combinato con funzioni di attivazione \red{saturanti} (come \hyperref[par:sigmoid]{sigmoid} e \red{tanh}) può portare a correzioni quasi nulle del gradiente e quindi la rete tende a non convergere.
\emptyln{}
...
\begin{example}{}{}
    Considerando l'aggiornamento dei pesi di \hyperref[sub:stochastic_gradient_descent]{SGD}:
    \[
...
\hyperref[sub:stochastic_gradient_descent]{SGD} con mini-batch può determinare una discesa del gradiente a \textbf{zig-zag} che rallenta la convergenza e la rende meno stabile: la discesa del gradiente può essere vista come la traiettoria di una biglia che rotola su una superficie sconnessa.
...
Oltre a \hyperref[sub:momentum]{Momentum} esistono altri ottimizzatori per l'aggiornamento dei pesi che possono accelerare la convergenza nella discesa del gradiente:
\begin{itemize}
    \item \ac{nag};
    \item \ac{adagrad};
    \item \red{Adadelta};
...
\red{Adam} è considerato lo stato dell'arte, anche se non sempre fornisce risultati del più classico \hyperref[sub:stochastic_gradient_descent]{SGD} con \hyperref[sub:momentum]{momentum}.
\begin{cimage}
