...
\begin{definition}{Lunghezza di una stringa}{lunghezza_str}
    ...
    \begin{itemize}
        \item $\ab w$ denota la lunghezza della stringa $w$
    \end{itemize}
\end{definition}
%
...
\begin{example}{Un automa $A$ che accetta $L = \pg{x01y: x, y \in \pg{0, 1}^*}$}{}
    ...
    \begin{center}
        \includegraphics[width=.6\textwidth]{assets/diag_trans.png}
        \captionof{figure}{Diagramma di transizione di un automa}\label{fig:diag_trans}
    \end{center}
\end{example}
%
...
\end{enumerate}
L'automa a stati finiti in \cref{fig:diag_trans}, accetta la stringa \tt{01101}.
\emptyln{}
La funzione di transizione $\dt$ può essere estesa a $\hdt$ che opera su stati e stringhe (invece che su stati e simboli).
\begin{flushleft}
    \begin{tabular}{ll}
        \bf{Base}: &$\hdt\pt{q, \eps} = q$ \\
        \bf{Induzione}: &$\hdt\pt{q, xa} = \dt\pt{\hdt\pt{q, x}, a}$
    \end{tabular}
\end{flushleft}
\begin{definition}{Linguaggi Regolari}{reg_lang}
    Formalmente, il \it{linguaggio accettato} da un automa a stati finiti deterministico $A$ è:
    \begin{equation*}
        L(A) = \pg{w: \hdt\pt{\qlo, w} \in F}
    \end{equation*}
    I linguaggi accettati da automi a stati finiti sono detti \bf{linguaggi regolari}
\end{definition}
